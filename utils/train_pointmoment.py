...
from model.pre_dgcnn_partseg import DGCNN_partseg

"""
time:05/23
version:v1
author:hanxinxin
description: +the module of selecting best acc on pointmoment
...
"""
def get_arguments():
    parser = argparse.ArgumentParser(description="Pretrain a pointnet/dgcnn model with PointMoment", add_help=False)
    ...
    # Checkpoints
    parser.add_argument("--exp-dir", type=Path, default="path/to/experiment",
    ...
    parser.add_argument('--lambd', default=0.5, type=float, metavar='L',
                        help='weight on lambda terms')
    parser.add_argument('--lambd1', default=1, type=float, metavar='L',
                        help='weight on lambda terms')
    parser.add_argument('--lambd2', default=1, type=float, metavar='L',
                        help='weight on lambda terms')
    parser.add_argument('--lambd3', default=1, type=float, metavar='L',
                        help='weight on lambda terms')
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    wandb.init(project="Ablation Analysis", name="two_pointnet_0.1")
    ...
    optimizer = opt.Adam(model.parameters(), lr=0.001, weight_decay=1e-6)
    ...
    # load checkpoint
    if (args.exp_dir / "model_.pth").is_file():
        if args.rank == 0:
            print("resuming from checkpoint")
        ckpt = torch.load(args.exp_dir / "model_.pth", map_location="cpu")
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if args.rank == 0:
            ...
            torch.save(model.backbone.state_dict(), args.exp_dir /"backbone/model_{:03d}.pth".format(epoch))
...
class PointHome(nn.Module):
    ...
    def __init__(self, args):
        ...
        # projector
        # sizes = [2048] + list(map(int, args.projector.split('-')))  # 2048-DGCNN 1024-pointnet 1024-DGCNN_partseg
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser('PointMoment training script', parents=[get_arguments()])
